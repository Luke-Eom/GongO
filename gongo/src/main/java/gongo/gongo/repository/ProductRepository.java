...
public interface ProductRepository extends JpaRepository<Product, Integer>{
    ...
    // 검색어로 조회 로직
    @Query("select p from Product as p inner join product_info as pi on p.name = pi.name where upper(p.name) like upper('%:input%') or upper(pi.brand) like upper('%:input%')")
    ArrayList<Product> findProductByInput(@Param("input") String input);
